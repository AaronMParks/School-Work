...
bool IntSet::isEmpty() const { return used == 0; }
...
void IntSet::DumpData(
    ostream &out) const { // already implemented ... DON'T change anything
  if (used > 0) {
    out << data[0];
...
{
  IntSet intersectIntSet = *this;
  int sizeOtherIntSet = otherIntSet.size();
  for (int i = 0; i < sizeOtherIntSet; ++i)
 {
   if (!otherIntSet.contains(data[i]))
      intersectIntSet.remove(data[i]);
 }
...
{
  IntSet subtractIntSet = *this;
  int sizeOtherIntSet = otherIntSet.size();
  for (int i = 0; i < sizeOtherIntSet; ++i)
 {
   if (!subtractIntSet.contains(otherIntSet.data[i]))
      subtractIntSet.remove(otherIntSet.data[i]);
 }
